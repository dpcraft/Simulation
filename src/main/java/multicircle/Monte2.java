...
//        }
        Thread threadA = new Thread(new myRunnable2(1, maxPoisonNo[0], nodeNum, monte));
        Thread threadB = new Thread(new myRunnable2(maxPoisonNo[0] + 1, maxPoisonNo[1], nodeNum, monte));
        Thread threadC = new Thread(new myRunnable2(maxPoisonNo[1] + 1, maxPoisonNo[2], nodeNum, monte));
        Thread threadD = new Thread(new myRunnable2(maxPoisonNo[2] + 1, maxPoisonNo[3], nodeNum, monte));
        threadA.start();
        threadB.start();
        threadC.start();
        threadD.start();
    ...
    public static Float[] silmu(int nodeNum, int circleNum) {
        ...
        for (int poisonNum = 1; poisonNum <= maxPoisonNum; poisonNum++) {
            result[poisonNum] = 0f;
            x[poisonNum] = poisonNum;
            for (int i = 0; i < experimentTimes; i++) {
                ArrayList<Integer> poisonNo = Utils.getRandom(nodeNum, poisonNum);
//                System.out.println("污染数据为：");
//                for (Integer p:poisonNo
//                     ) {
//                    System.out.print(p + ", ");
//                }
//                System.out.println();
                if (Utils2.check(nodeNum, circleNum, poisonNo) >= 1) {
                    result[poisonNum]++;
                }
            }
...
class myRunnable2 implements Runnable{
    ...
    public myRunnable2(Integer startIndex, Integer endIndex, Integer nodeNum, Monte2 monte) {
